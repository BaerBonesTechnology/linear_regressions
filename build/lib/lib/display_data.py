# @title Plot Model Data using linear regression
...
# constants
from lib.constants.labels import *
from lib.constants.graph_type import *
...
def make_plots(df, feature_names, label_name, model_output, sample_size=200):
    ...
    model_plot_type = SCATTER_GRAPH if is_2d_plot else SURFACE_GRAPH
    fig = make_subplots(rows=1, cols=2, subplot_titles=("Loss Curve", "Model Plot"),
                        specs=[[{"type": SCATTER_GRAPH}, {"type": model_plot_type}]])
...
def plot_loss_curve(epochs, rmse, fig):
    ...
    fig.update_xaxes(title_text=epoch_label, row=1, col=1)
    fig.update_yaxes(title_text=rmse_label, row=1, col=1)
...
def plot_data(df, feature_names, label_name, fig):
    ...
    if len(feature_names) == 1:
        ...
        fig.update_layout(scene1=dict(xaxis_title=feature_names[0], yaxis_title=feature_names[1],
