...
def runExperiment(data, feature_names, label_name, learning_rate, epochs, batch_size):
    ...
    model = BuildModel(learning_rate=learning_rate, num_of_features=num_of_features)
    model_output = train_model(model=model, data=data, features=features, label=label, epochs=epochs, batch_size = batch_size)
    print('\nSUCCESS: training experiment complete\n')
    print('{}'.format(data, feature_names, label_name, model_output))
    dd.make_plots(data, feature_names, label_name, model_output)
